...
head(data)
setnames(data, "t1", "team")
setnames(data, "descr", "goals")
setnames(data, "set_id", "Id")
...
groupByYearPiece <- function(dt, minYear, maxYear, minPiece,
    ...
    result <- dt %>% filter(year >= minYear, year <= maxYear,
                            points >= minPiece, points <= maxPiece,
                            team %in% themes) 
    return(result)
}
...
groupByPieceThemeAvg <- function(dt,  minYear, maxYear, minPiece,
    ...
    result <- dt %>% 
            group_by(team) %>%
            summarise(avgPieces = mean(points)) %>%
            arrange(team)
    return(result)
}
...
plotPiecesByYear <- function(dt, dom = "piecesByYear", 
  ...
  return '<h5><b>Team</b>: ' + e.point.name  
    + '</h5>'
